...
## 6. 決定事項（2026-02-02 確定）

実装前に決定した事項:

1. **日付フィルタの詳細仕様**
   - タイムゾーン: **処理なし（Dataset内の日付データをそのまま使用）**
   - Dataset に "2024-01-01" があれば、それを "2024-01-01" として扱う（変換なし）
   - 境界: **開始日・終了日ともに含む** （2024-01-01 ～ 2024-01-31 = 31日間）
   - データ型: **ISO 8601（YYYY-MM-DD）のみ**
   - 理由: データの整合性確保、タイムゾーン変換によるズレ回避、実装シンプル化。

2. **許可ライブラリのホワイトリスト**
   - 確定: pandas==2.2.0, pyarrow==15.0.0, plotly==5.18.0, numpy==1.26.3, scipy==1.11.4, matplotlib==3.8.2, python-dateutil==2.8.2
   - scikit-learn, xgboost 等は V1+ で検討予定
   - 理由: 数値計算・統計分析が充実。大半の分析ユースケースに対応可能。イメージサイズ ~1.0GB。

3. **S3認証方式**
   - **IAMロール（ECSタスク）経由で一元管理**
   - API/Executor タスクに IAM ロールをアタッチし、STS で一時クレデンシャル取得
   - ユーザはS3バケット/パスを指定、API が代わりにアクセス
   - 理由: AWS ネイティブでセキュア。ユーザ側の認証情報管理コスト不要。

4. **HTMLカードで許可するJSライブラリ**
   - **社内CDN配信・バージョン固定方式**
   - `https://cdn.internal.company.com/plotly/2.27.0/plotly.min.js` 等で固定配置
   - CSP: `script-src 'unsafe-inline' https://cdn.internal.company.com`
   - バージョン更新は CI/CD フロー化
   - 理由: 管理・セキュリティが明確。全カード・ユーザで同一バージョン使用 → 互換性リスク低い。

5. **相対日付フィルタ（過去7日等）**
   - **MVP から外す** → V1以降の計画に移行
   - 理由: 絶対日付範囲で基本要件は満たされ、実装・テストが複雑化するため優先度低い。
