...
## 11. 決定事項（2026-02-02 確定）

### 日付フィルタ仕様（決定）

**タイムゾーン**: 処理なし（Dataset基準）
- Dataset内の日付データをそのまま使用
- Dataset に "2024-01-01" があれば、それを "2024-01-01" として扱う（変換なし）
- データの整合性確保、タイムゾーン変換によるズレ回避

**日付範囲の境界**: 開始・終了日ともに含む
- 例：2024-01-01 ～ 2024-01-31 = 31日間（直感的）

**入力形式**: ISO 8601（YYYY-MM-DD）のみ
- フロントエンド: カレンダーUI → YYYY-MM-DD形式
- バックエンド: Datasetの日付カラムと直接比較
- 緩い形式解析は不対応
...
**V1+ で検討**:
- seaborn
- scikit-learn, xgboost（機械学習ライブラリ）

### S3認証方式（決定）

**方式**: IAMロール（ECS タスク）経由で一元管理

**構成**:
- API/Executor ECS タスクに IAM ロールをアタッチ
- STS AssumeRole で一時クレデンシャル取得
- ユーザは S3 バケット/パスを指定、API が代わりにアクセス
- ユーザ側の認証情報管理コスト不要
...
**方式**: 社内 CDN 配信・バージョン固定

**配置**:
- `https://cdn.internal.company.com/plotly/2.27.0/plotly.min.js`
- S3/CloudFront でホスト
