...
interface TransformListProps {
  ...
  onEdit: (transform: Transform) => void
  onDelete: (transform_id: string) => void
  pagination?: TransformListResponse['pagination']
          ...
          >
            <div className="flex justify-between items-start mb-2">
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => onDelete(transform.transform_id)}
            ...
            <div className="text-sm text-gray-600 space-y-1 mb-4">
              <p>入力Dataset数: {transform.input_dataset_ids.length}</p>
              {transform.output_dataset_id && <p>出力Dataset: {transform.output_dataset_id}</p>}
              {transform.schedule && <p>スケジュール: {transform.schedule}</p>}
              {transform.last_executed_at && (
                <p>最終実行: {new Date(transform.last_executed_at).toLocaleString('ja-JP')}</p>
              )}
            </div>
          </div>
        ))}
